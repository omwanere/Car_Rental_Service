...
const ListingCard = ({
  listingId,
  creatorId, // Only the creator's _id
  creatorName, // Optional - if you want to show in UI
  listingPhotosPaths,
  city,
  province,
  country,
  ...
  const patchWishList = async () => {
    if (user?._id !== creatorId) {
      // Now using creatorId directly
      const response = await fetch(
        `http://localhost:8000/api/v1/users/${user?._id}/wishlist/${listingId}`,
        {
          method: "PATCH",
          headers: {
